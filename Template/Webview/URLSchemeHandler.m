    ...
    if ([scheme isEqualToString:@"app"]) {
        if ([stringToLoad hasPrefix:@"/_app_file_"]) {
            ...
            if ([stringToLoad isEqualToString:@""] || [url.pathExtension isEqualToString:@""]) {
                startPath = [startPath stringByAppendingPathComponent:@"你的首页地址"];
            } else {
...
{
    NSArray * mediaExtensions = @[@"m4v", @"mov", @"mp4", @"aac", @"ac3", @"aiff", @"au", @"flac", @"m4a", @"mp3", @"wav"];
    return [mediaExtensions containsObject:pathExtension.lowercaseString];
}
